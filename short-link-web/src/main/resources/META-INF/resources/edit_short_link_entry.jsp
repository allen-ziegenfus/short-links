	...
	<aui:form action="<%= updateShortLinkEntryURL %>" method="post" name="fm">
		...
		<aui:fieldset>
			...
			%>
				...
				<aui:input checked="<%= httpStatusCode == shortLinkEntry.getHttpStatusCode() %>" id="httpStatusCode" label="<%= httpStatusCode + StringPool.SPACE + HttpStatus.getStatusText(httpStatusCode) %>" name="httpStatusCode" type="radio" value="<%= httpStatusCode %>" />
