...
@Component(
	...
	property = {
		...
		"mvc.command.name=updateShortLinkEntry"
	},
	service = MVCActionCommand.class
)
public class UpdateShortLinkEntrySubmitAction extends BaseMVCActionCommand {
	...
	protected void doProcessAction(
		...
		int httpStatusCode = ParamUtil.getInteger(
			actionRequest, "httpStatusCode");
		boolean active = ParamUtil.getBoolean(actionRequest, "active");

		if (shortLinkEntryId > 0) {
			ShortLinkEntryLocalServiceUtil.updateShortLinkEntry(
				shortLinkEntryId, originalURL, shortURL, httpStatusCode,
				active);
		}
		else {
